...
class AssasDatabaseManager:
    ...
    def set_hdf5_size_by_uuid(
        ...
        update = {f'system_size_hdf5': f'{str(size)}'}
        logger.info(f'Update file document with uuid {uuid} with update string {update}.')
    ...
    def get_overall_database_size(
        ...
        dataframe['system_size_bytes'] = dataframe['system_size'].apply(self.convert_to_bytes)
    ...
    def update_archive_sizes(
        ...
        if len(document_file_list) == 0:
            ...
            logger.info(f'No archives in state UPLOADED without updated binary size present.')
    ...
    def register_archives(
        ...
        for idx, archive in enumerate(archive_list):
            ...
            document_file.set_value('system_size_hdf5', AssasDatabaseManager.get_file_size(archive.result_path))
    ...
    def convert_next_validated_archive(
        ...
        try:
            ...
            document_file.set_value('system_status', AssasDocumentFileStatus.VALID)
            document_file.set_value('system_size_hdf5', AssasDatabaseManager.get_file_size(document_file.get_value('system_result')))
    ...
    def update_meta_data_of_valid_archives(
        self
    )-> None:
